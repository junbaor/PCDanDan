...
-(void)initSelf{
    ...
    scrollFrame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, self.frame.size.height);
    ...
    [self addSubview:scrollView];
//    if(_direction == CycleDirectionPortait)
//    {
//      scrollDirection = CycleDirectionPortait;
//    }else{
...
- (void)refreshScrollView {
    ...
    if ([_dataSource respondsToSelector:@selector(frameOfCycleScrollView:)]) {
        scrollFrame = [_dataSource frameOfCycleScrollView:self];
        scrollView.frame = scrollFrame;
    }
    if ([_dataSource respondsToSelector:@selector(numberOfViewsInCycleScrollView:)]) {
        totalPage = (int)[_dataSource numberOfViewsInCycleScrollView:self];
    }
    if (totalPage==0) {
        return;
    }
    // 在水平方向滚动
    if(_direction == CycleDirectionLandscape) {
    ...
    if (_direction == CycleDirectionLandscape && totalPage>1) {
        [scrollView setContentOffset:CGPointMake(scrollFrame.size.width, 0)];
    }
    if (_direction == CycleDirectionPortait && totalPage>1) {
...
-(UIView *)getViewAtPage:(int)page{
    ...
    if ([_dataSource respondsToSelector:@selector(cycleScrollView:viewAtPage:)]) {
        ...
        if (_cacheSize>0) {
//            [_cacheViewsDict setValue:view forKey:[NSIndexPath indexPathWithIndex:page]]; //此处修改
            [_cacheViewsDict setValue:view forKey:(NSString *)[NSIndexPath indexPathWithIndex:page]];
        }
        return view;
    }
