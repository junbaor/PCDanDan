...
- (void)_setupSubviews
{
    ...
    //输入框
    _inputTextView = [[EaseTextView alloc] initWithFrame:CGRectMake(self.horizontalPadding, self.verticalPadding, self.frame.size.width - self.verticalPadding * 2, self.frame.size.height - 10)];
...
//    [styleChangeButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_keyboard"] forState:UIControlStateSelected];
    ...
    [styleChangeButton setTitle:@"投注" forState:UIControlStateNormal];
    [styleChangeButton setBackgroundColor:RGB(254, 199, 57)];
    [styleChangeButton setTitleColor:RGB(108, 28, 7) forState:UIControlStateNormal];
    [styleChangeButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateSelected];
    styleChangeButton.titleLabel.font = [UIFont systemFontOfSize:13];
    styleChangeButton.layer.masksToBounds =YES;
    styleChangeButton.layer.cornerRadius = 3;
...
- (void)setInputViewLeftItems:(NSArray *)inputViewLeftItems
{
    ...
    CGFloat itemHeight = self.toolbarView.frame.size.height - 14;
        ...
        if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
            ...
            if (chatItem.button) {
                ...
                if (itemFrame.size.width == 0) {
                    itemFrame.size.width = itemFrame.size.height+25;
                }
...
- (void)styleButtonAction:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(didTouZhuButtonAction:)])
    {
        [self.delegate didTouZhuButtonAction:button];
    }
