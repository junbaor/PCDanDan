//
//  BianZhangRecordViewController.m
//  PCDanDan
//
...
#import "ZBRecordListInfo.h"

@interface BianZhangRecordViewController ()
{
...
@implementation BianZhangRecordViewController
...
- (void)httpGetRecordList
{
    ...
    __weak BianZhangRecordViewController *weakSelf = self;
    [helper getZBRecordListInfoWithUserId:[ShareManager shareInstance].userinfo.id
...
- (void)handleloadResult:(NSDictionary *)resultDic
{
    ...
    if (resourceArray && resourceArray.count > 0)
    {
        ...
        {
            ZBRecordListInfo *info = [dic objectByClass:[ZBRecordListInfo class]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZBRecordListInfo *info = [dataSourceArray objectAtIndex:indexPath.row];
    
    cell.oneLabel.text = [Tool timeStringToDateSting:info.create_time format:@"yyyy-MM-dd HH:mm:ss"];
    cell.twoLabel.text = [NSString stringWithFormat:@"%.2f元宝",info.point];
    if (info.point > 0) {
        cell.twoLabel.textColor = RGB(235, 82, 83);
    }else{
        cell.twoLabel.textColor = RGB(0, 128, 64);
    }
    cell.threeLabel.text = info.point_desc;
