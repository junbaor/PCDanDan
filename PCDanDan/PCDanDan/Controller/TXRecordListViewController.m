...
#import "TXRcordListInfo.h"
#import "ZhangBianListTableViewCell.h"

@interface TXRecordListViewController ()
{
...
@implementation TXRecordListViewController
...
- (void)httpGetRecordList
{
    ...
    __weak TXRecordListViewController *weakSelf = self;
    [helper getTXRecordListInfoWithUserId:[ShareManager shareInstance].userinfo.id
...
- (void)handleloadResult:(NSDictionary *)resultDic
{
    ...
    NSArray *resourceArray = [[resultDic objectForKey:@"data"] objectForKey:@"list"];
    ...
    if (resourceArray && resourceArray.count > 0)
    {
        ...
        {
            TXRcordListInfo *info = [dic objectByClass:[TXRcordListInfo class]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TXRcordListInfo *info = [dataSourceArray objectAtIndex:indexPath.row];
    
    cell.oneLabel.text = [Tool timeStringToDateSting:info.create_time format:@"yyyy-MM-dd HH:mm:ss"];
    cell.twoLabel.text = [NSString stringWithFormat:@"%.2f元",info.fee];
    
    if([info.status intValue] == 0)
    {
        cell.threeLabel.text = @"提现中";
    }else if([info.status intValue] == 1){
        cell.threeLabel.text = @"成功";
    }else{
        cell.threeLabel.text = @"失败";
    }
